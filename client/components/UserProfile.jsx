    ...
    <div className='bg-scroll bg-cover' style={{ backgroundImage: `url(${profileImage})` }}>
      <div className='container h-screen inline'>
        ...
        <form className='flex items-center grid grid-cols-1 grid-rows-6 w-3/12 h-64' id='profile-form'>
          ...
          <input className='rounded-md shadow-sm col-1 h-12 ml-20' type='text' id='email' name='email' placeholder='Enter Email Address' onChange={(e) => updateEmail(e.target.value)}/>      
          <label  className='ml-20' for='phone'>Phone Number</label>
          <input className='rounded-md shadow-sm col-1 h-12 ml-20' type='int' id='phone' name='phone' placeholder='Enter Phone Number' onChange={(e) => updatePhoneNumber(e.target.value)}/>      
        </form>
